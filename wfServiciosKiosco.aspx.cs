...
public partial class wfServiciosAutogestion : System.Web.UI.Page
{
    ...
    {
        DataSet dsServiciosConsultas = new DataSet();
        clsblParametricas blParam = new clsblParametricas();
        String msgError;
        String strSQL = "";

        msgError = blParam.ConsultaServiciosConsultaKiosco(ref dsServiciosConsultas, "", tbNombre.Text);
        if (msgError == "")
        {
            gvServiciosAuto.DataSource = dsServiciosConsultas;
    ...
    protected void gvServiciosAuto_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName.ToString() == "ADMINISTRAR")
            Response.Redirect("wfServicioKiosco.aspx?id=" + idS);
        if (e.CommandName.ToString() == "CONSULTAR")
            Response.Redirect("wfServicioKiosco.aspx?consulta=si&id=" + idS);
    }
    ...
    protected void btnAgregar_Click1(object sender, EventArgs e)
    {
        Session["tbnombre"] = tbNombre.Text;
        Session["txSQL"] = txSQL.Text;
        Response.Redirect("wfServicioKiosco.aspx");
    }
}