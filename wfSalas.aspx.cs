...
public partial class wfOficinas : System.Web.UI.Page
{
    ...
    private void Filtrar()
    {
        ...
        msgError = blParam.ConsultarSalas(ref dsOficinas, "", tbNombre.Text);
        if (msgError == "")
        {
            gvSalas.DataSource = dsOficinas;
            gvSalas.DataBind();

            if (gvSalas.Rows.Count <= 0)
            {
            ...
            {
                tbNoRegistros.Text = gvSalas.Rows.Count.ToString();
    ...
    protected void gvOficinas_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName.ToString() == "ADMINISTRAR")
            Response.Redirect("wfSala.aspx?id=" + idS);
        if (e.CommandName.ToString() == "CONSULTAR")
            Response.Redirect("wfSala.aspx?consulta=si&id=" + idS);
    }
