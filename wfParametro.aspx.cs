...
public partial class wfParametro : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!((objUsuario.PermisoModulo("59", Session["IDUSUARIO"].ToString(), "I")) || (objUsuario.PermisoModulo("59", Session["IDUSUARIO"].ToString(), "U"))))
        {
            btnGrabar.Visible = false;
            objUsuario.SoloLectura(this);
        }
        if (!objUsuario.PermisoModulo("59", Session["IDUSUARIO"].ToString(), "D"))
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (hfid.Value != "")
            {
                msgError = blPara.ConsultaParametrosGenerales(ref dsParametros, hfid.Value, "");
                if (msgError == "")
                {
                    tbParametro.Text = dsParametros.Tables[0].Rows[0]["Parametro"].ToString();
                    tbValor.Text = dsParametros.Tables[0].Rows[0]["Valor"].ToString();
                }
            }
            else
                btnEliminar.Visible = false;
    ...
    protected void btnGrabar_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            blObj.NombreTabla = "Parametros_Generales";
            blObj.Add("Parametro", tbParametro.Text);
            blObj.Add("Valor", tbValor.Text);
            if (hfid.Value == "")
            {
    ...
    protected void btnEliminar_Click(object sender, EventArgs e)
    {
        ...
        {
            lbConfirmacion.Text = "¡Error eliminando el parámetro!" + ex.Message;
            notificacion.Attributes["Class"] = "alert alert-dismissible alert-danger";
            notificacion.Visible = true;
        }
    }
    ...
    {
        Session["Volver"] = "S";
        Response.Redirect("wfParametros.aspx");
    }
}