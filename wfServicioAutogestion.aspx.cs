...
public partial class wfServicioAutogestion : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        String msgError;
        DataSet dsServiciosAtencion = new DataSet();
        ...
        if (!Page.IsPostBack)
        {
            ...
            if (hfid.Value != "")
            {
                msgError = blPara.ConsultaServiciosAtencion(ref dsServiciosAtencion, hfid.Value, "");
                if (msgError == "")
                {
                    tbNombre.Text = dsServiciosAtencion.Tables[0].Rows[0]["Nombre"].ToString();
                    tbUrl.Text = dsServiciosAtencion.Tables[0].Rows[0]["Url"].ToString();

                    cbHabilitaServicio.Checked = false;
                    if (dsServiciosAtencion.Tables[0].Rows[0]["Activo"].ToString() != "")
                    {
                        if ((bool)dsServiciosAtencion.Tables[0].Rows[0]["Activo"])
                            cbHabilitaServicio.Checked = true;
                    }

                    //                    tbLongitud.Text = dsOficinas.Tables[0].Rows[0]["Longitud"].ToString();
                    //                    tbLatitud.Text = dsOficinas.Tables[0].Rows[0]["Latitud"].ToString();
                }
            }
       //     else
    //            btnEliminar.Visible = false;
   ...
   //         btnEliminar.Visible = false;
    ...
    protected void btnGrabar_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            blObj.NombreTabla = "Servicios_Autoaten";
            blObj.Add("Nombre", tbNombre.Text);
            blObj.Add("Url", tbUrl.Text);
            if (cbHabilitaServicio.Checked)
                blObj.Add("Activo", "1");
            else
                blObj.Add("Activo", "0");

            //            blObj.Add("Longitud", tbLongitud.Text);
            //            blObj.Add("Latitud", tbLatitud.Text);
            if (hfid.Value == "")
            {
...
//            btnEliminar.Visible = false;
    ...
    {
        Session["Volver"] = "S";
        Response.Redirect("wfServiciosAutogestion.aspx");
    }
}