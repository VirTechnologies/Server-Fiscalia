...
public partial class wfServiciosTipoAtencion : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!(objUsuario.PermisoModulo("55", blU.ValorObjetoString(Session["IDUSUARIO"]), "I")))
            btnAgregar.Visible = false;
        //if (txSQL.Text != "")
            Filtrar();
        if (blU.ValorObjetoString(Session["Volver"]) == "S")
        {
            tbServicio.Text = Session["tbServicio"].ToString();
    ...
    private void Filtrar()
    {
        ...
        msgError = blParam.ConsultaServiciosTiposDeAtencion(ref dsServicios, "", tbServicio.Text);
        if (msgError == "")
        {
    ...
    protected void gvServicios_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (!obj.PermisoModulo("55", Session["IDUSUARIO"].ToString(), "U"))
    ...
    protected void gvServicios_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName != "")
        {
            ...
            Session["tbServicio"] = tbServicio.Text;
            Session["txSQL"] = txSQL.Text;
        }
        if (e.CommandName.ToString() == "ADMINISTRAR")
            Response.Redirect("wfServicioTipoAtencion.aspx?id=" + idS);
        if (e.CommandName.ToString() == "CONSULTAR")
            Response.Redirect("wfServicioTipoAtencion.aspx?consulta=si&id=" + idS);
    }
    ...
    protected void btnAgregar_Click(object sender, EventArgs e)
    {
        Session["tbServicio"] = tbServicio.Text;
        Session["txSQL"] = txSQL.Text;
        Response.Redirect("wfServicioTipoAtencion.aspx");
    }
