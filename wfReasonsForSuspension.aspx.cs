...
public partial class wfReasonsForSuspension : System.Web.UI.Page
{
    ...
    private void Filtrar()
    {
        ...
        msgError = blParam.ConsultaReasonsForSuspension(ref dsReasons, "", tbNombre.Text);
        if (msgError == "")
        {
            gvReasons.DataSource = dsReasons;
            gvReasons.DataBind();

            if (gvReasons.Rows.Count <= 0)
            {
            ...
            {
                tbNoRegistros.Text = gvReasons.Rows.Count.ToString();
    ...
    protected void gvReasons_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName.ToString() == "ADMINISTRAR")
            Response.Redirect("wfReasonForSuspension.aspx?id=" + idS);
        if (e.CommandName.ToString() == "CONSULTAR")
            Response.Redirect("wfReasonForSuspension.aspx?consulta=si&id=" + idS);
    }
    ...
    protected void btnAgregar_Click1(object sender, EventArgs e)
    {
        Session["tbnombre"] = tbNombre.Text;
        Session["txSQL"] = txSQL.Text;
        Response.Redirect("wfReasonForSuspension.aspx");
    }
}